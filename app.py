...
# Carbon conversion factors (Standard EPA / GHG Protocol estimates)
EMISSION_FACTORS = {
    'travel': {
...
# Average footprints (in tonnes of CO2e per year)
NATIONAL_AVERAGE = 16.0  # US/developed country average
GLOBAL_TARGET = 2.0      # Sustainable target per person
...
def scan_bill():
    # Simulate OCR bill reading
    if 'billFile' not in request.files and not request.form.get('fileName'):
        return jsonify({'error': 'No file uploaded'}), 400
        
    # Get filename to simulate a realistic parse based on input
    file_name = request.form.get('fileName', 'electricity_bill.pdf')
    
    # Mock OCR extraction
    # Give different readings for variety, but ensure it returns plausible numbers
    kwh_readings = [120, 245, 380, 410, 185]
    selected_kwh = random.choice(kwh_readings)
    
    # Calculate carbon footprint contribution of this bill
    # Monthly carbon footprint = kWh * emission factor
...
def chatbot():
    ...
    # Simple rule-based chatbot with a rich response dataset
    responses = [
        {
...
def leaderboard():
    # Mock leaderboard datasets for Office, School and Personal modes
    data = {
        'office': [
